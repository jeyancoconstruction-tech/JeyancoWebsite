{{-- The loading screen: the check that decides whether it runs, and its chrome.
     ...
     outside. A click inside the app, a form submit — Sign In included — and
     the back button all skip it and go straight to the page. (It was taken
     out altogether on 2026-09-23 along with the one Sign In used to raise;
     Michael wanted this one back. Sign In stays without.)
     ...
     And every class it styles is its own. The line art's used to be .fade,
     which is also Bootstrap's class for every dialog and tab: loaded on the
     app layout, it gave them a one-second transition, and Bootstrap waits for
     that before it finishes — a second on each tab switch, two to close a
     dialog. It is .jp-fade now. --}}
...
.jp-loading #jp-loader { display: grid; }
...
.jp-stage {
    ...
    animation: jp-rise .8s var(--jp-ease) both;
...
@keyframes jp-rise    { from { opacity: 0; transform: translateY(10px) scale(.97); } to { opacity: 1; transform: none; } }
...
/* ── The entrance the loader hands over to ───────────────────────────────
   ...
   .draw and .jp-fade are for line art: the outlines draw themselves in like
   a blueprint, and the detail arrives after them.

   Hidden only while the loader is up (.jp-loading), so a page reached from
   inside the app, or one whose scripts never ran, is simply there. The
   transitions apply only while the page is coming in (.jp-entering) and are
   let go afterwards, so the Sign In button keeps its own quick press and
   hover rather than the entrance's slow ones. */
:root { --jp-ease: cubic-bezier(.2, .8, .2, 1); }

.draw { stroke-dasharray: 1; }

.jp-loading .rv      { opacity: 0; transform: translateY(14px); }
.jp-loading .rv-left { transform: translateX(-18px); }
.jp-loading .draw    { stroke-dashoffset: 1; }
.jp-loading .jp-fade { opacity: 0; }
...
.jp-entering .rv, .jp-entering .draw, .jp-entering .jp-fade { transition-delay: calc(var(--jp-d, 0) * 1ms); }

/* A reader who has asked for less movement gets the screen and the page,
   and none of the choreography between them. */
@media (prefers-reduced-motion: reduce) {
    #jp-loader, #jp-loader *, .jp-entering .rv, .jp-entering .draw, .jp-entering .jp-fade {

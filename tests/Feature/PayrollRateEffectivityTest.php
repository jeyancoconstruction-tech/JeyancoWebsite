...
class PayrollRateEffectivityTest extends TestCase
{
    ...
    /**
     * Michael, 2026-09-26: the rate history table is off Payroll Settings,
     * since the Audit Logs already record every change. The dated rows stay
     * — they are what past days are paid under — and each save is logged.
     */
    public function test_the_rate_history_lives_in_the_audit_logs_not_on_settings(): void
    {
        foreach (['2026-01-01', '2026-02-01'] as $from) {
            $this->rate($from);
        }

        $page = $this->actingAs($this->admin())->get(route('settings.index', ['tab' => 'payroll']));
        $page->assertOk()->assertDontSee('Rate history')->assertSee('Audit Logs');
        $this->assertArrayNotHasKey('payrollRates', $page->original->getData());

        $before = PayrollRate::count();
        $this->actingAs($this->admin())
             ->post(route('payroll-rates.store'), ['effective_from' => '2026-10-01', 'uses_defaults' => 1])
             ->assertSessionHasNoErrors();

        $this->assertSame($before + 1, PayrollRate::count(), 'a change is still a new dated row');
        $this->assertTrue(
            \App\Models\AuditLog::where('module', 'Payroll Settings')->exists(),
            'and the change is in the Audit Logs'
        );
    }
